...
% make3column_1.m only writes regressor (.txt) files and includes missed
...
%     end
    ...
    header = {'subID','rt_gain','rt_loss','rt_nongain','rt_nonloss','rt_mean','gain_succ','gain_acc','loss_succ','loss_acc','nongain_succ','nongain_acc','nonloss_succ','nonloss_acc','total_acc','gain_count','loss_count','nongain_count','nonloss_count','missed_count','missed_percent','min_targettime', 'max_targettime','mean_targettime','Grange'};
    behavdata = [str2num(subID), mean(gain_rt), mean(loss_rt), mean(nongain_rt), mean(nonloss_rt), mean(all_rt), gain_succ, gain_acc, lose_succ, lose_acc, nongain_succ, nongain_acc, nonloss_succ, nonloss_acc, total_acc, gain_count, lose_count, nongain_count, nonloss_count, missed_count, missed_percent, min(target_dur), max(target_dur), mean(target_dur), raw{6,10}];
