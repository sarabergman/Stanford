% make3column_1.m writes 3 column files (.txt) and they include trials for which there was no button press (missed trials).
% other things (e.g., counts, successful trials) are calculated but are
% never ouput. a missed.txt file may get created, but those trials are not
% removed from other regressors. model4 does not account for missed trials.
...
for s = 1:length(sublist)
    ...
    if strcmp(subID,'002')
        trigtime = raw{2,41};
        range = 3:74;
    elseif strcmp(subID, '021') || strcmp(subID,'029')  %only 70 trials
