...
% unsucc_gain (formerly grouped with nongain), and succ_avoid_loss
% (formerly nonloss).
...
for s = 1:length(sublist)
    ...
    model5dir = fullfile(maindir, 'Analysis', 'EV', subID, 'model5'); %where EV files get dumped for excluding missed trials
    if ~exist (model5dir)
        mkdir (model5dir)
    end
...
%     end
    ...
    all_rt = [gain_rt loss_rt nongain_rt nonloss_rt];
    
    header = {'subID','rt_gain','rt_loss','rt_nongain','rt_nonloss','rt_mean','gain_succ','gain_acc','loss_succ','loss_acc','nongain_succ','nongain_acc','nonloss_succ','nonloss_acc','total_acc','gain_count','loss_count','nongain_count','unsucc_gain_count','nonloss_count','succ_avoid_loss_count','missed_count','missed_percent','min_targettime', 'max_targettime','mean_targettime','Grange'};
    behavdata = [str2num(subID), mean(gain_rt), mean(loss_rt), mean(nongain_rt), mean(nonloss_rt), mean(all_rt), gain_succ, gain_acc, lose_succ, lose_acc, nongain_succ, nongain_acc, nonloss_succ, nonloss_acc, total_acc, gain_count, lose_count, nongain_count, unsucc_gain_count, nonloss_count, succ_avoid_loss_count, missed_count, missed_percent, min(target_dur), max(target_dur), mean(target_dur), raw{6,10}];
    ...
    rt(1:length(nongain_rt),3) = nongain_rt;
    rt(1:length(nonloss_rt),4) = nonloss_rt;
