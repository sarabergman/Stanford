...
%%anticipation of gain > nongain, run this script.
...
%     '101','102','103','104','107'};  
sublist = {'091','098','102','103','106','107','108','112','113','114','116','121'};  
...
for s = 1:length(sublist)
    ...
    PPIdir = fullfile(maindir, 'Analysis', 'EV', subID, 'PPI.antgain'); %where EV files get dumped for 
    if ~exist (PPIdir)
        mkdir (PPIdir)
    end
    ...
    gainVnongain_ant_time = [];     %7. regressor to be used for PPI
    ...
    nongain_ant_dur = [];
    nonloss_ant_dur = [];
    gainVnongain_ant_dur = []; 
    
    gainVnongain_ant_weight = [];
    gainandnongain_ant_weight = [];
    ...
    for i = range  %i is the row, so trials are from row 4-75; 3:74 for 002
        ...
        if strcmp(raw{i,62},'dist_resized/cir.bmp') || strcmp(raw{i,62},'dist_resized/sqr.bmp')  %if cir OR sqr
            ...
            if strcmp(raw{i,62},'dist_resized/cir.bmp')  %if cir, no gain (nongain)
                ...
                gainVnongain_ant_time = [gainVnongain_ant_time (raw{i,44} - adjust)/1000]; %time for the PPI (ant nongain)
                gainVnongain_ant_dur = [gainVnongain_ant_dur (raw{i,63}-raw{i,44})/1000]; %duration for the PPI (ant nongain)
                gainVnongain_ant_weight = [gainVnongain_ant_weight -1];   %weight for the PPI (ant nongain)
                gainandnongain_ant_weight = [gainandnongain_ant_weight 1]; 
        ...
        elseif strcmp(raw{i,62},'dist_resized/cir2.bmp')  %if cir2
            ...
            gainVnongain_ant_time = [gainVnongain_ant_time (raw{i,44} - adjust)/1000]; %time for the PPI (ant gain)
            gainVnongain_ant_dur = [gainVnongain_ant_dur (raw{i,63}-raw{i,44})/1000]; %duration for the PPI (ant gain)
            gainVnongain_ant_weight = [gainVnongain_ant_weight 1];   %weight for the PPI (ant gain)
            gainandnongain_ant_weight = [gainandnongain_ant_weight 1]; 
...
%     end
    ...
    %PPI regressor for ant gain>nongain
    ant_gainVnongain = [gainVnongain_ant_time' gainVnongain_ant_dur' gainVnongain_ant_weight']; 
    ant_gain_nongain = [gainVnongain_ant_time' gainVnongain_ant_dur' gainandnongain_ant_weight'];
    ...
    dlmwrite([subID '_ant_gain_V_nongain.txt'], ant_gainVnongain, 'delimiter','\t','newline','unix', 'precision', 7);
    dlmwrite([subID '_ant_gain_and_nongain.txt'], ant_gain_nongain, 'delimiter','\t','newline','unix', 'precision', 7);
