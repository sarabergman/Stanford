...
for s = 1:length(sublist)
    ...
    PPIdir = fullfile(maindir, 'Analysis', 'EV', subID, 'PPI.antnongain'); %where EV files get dumped for 
    if ~exist (PPIdir)
        mkdir (PPIdir)
    end
    ...
    nongainVgain_ant_time = [];     %7. regressor to be used for PPI
    ...
    nongain_ant_dur = [];
    nonloss_ant_dur = [];
    nongainVgain_ant_dur = []; 
    
    nongainVgain_ant_weight = [];
    ...
    for i = range  %i is the row, so trials are from row 4-75; 3:74 for 002
        ...
        if strcmp(raw{i,62},'dist_resized/cir.bmp') || strcmp(raw{i,62},'dist_resized/sqr.bmp')  %if cir OR sqr
            ...
            if strcmp(raw{i,62},'dist_resized/cir.bmp')  %if cir, no gain (nongain)
                ...
                nongainVgain_ant_time = [nongainVgain_ant_time (raw{i,44} - adjust)/1000]; %time for the PPI (ant nongain)
                nongainVgain_ant_dur = [nongainVgain_ant_dur (raw{i,63}-raw{i,44})/1000]; %duration for the PPI (ant nongain)
                nongainVgain_ant_weight = [nongainVgain_ant_weight 1];   %weight for the PPI (ant nongain)
        ...
        elseif strcmp(raw{i,62},'dist_resized/cir2.bmp')  %if cir2
            ...
            nongainVgain_ant_time = [nongainVgain_ant_time (raw{i,44} - adjust)/1000]; %time for the PPI (ant gain)
            nongainVgain_ant_dur = [nongainVgain_ant_dur (raw{i,63}-raw{i,44})/1000]; %duration for the PPI (ant gain)
            nongainVgain_ant_weight = [nongainVgain_ant_weight -1];   %weight for the PPI (ant gain)
            gainandnongain_ant_weight = [gainandnongain_ant_weight 1]; 
...
%     end
    ...
    ant_nongainVgain = [nongainVgain_ant_time' nongainVgain_ant_dur' nongainVgain_ant_weight']; 
    ant_gain_nongain = [nongainVgain_ant_time' nongainVgain_ant_dur' gainandnongain_ant_weight'];
    ...
    dlmwrite([subID '_ant_nongain_V_gain.txt'], ant_nongainVgain, 'delimiter','\t','newline','unix', 'precision', 7);
